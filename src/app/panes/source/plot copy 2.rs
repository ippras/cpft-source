...
impl PlotView<'_> {
    ...
    fn try_show(&mut self, ui: &mut Ui) -> PolarsResult<()> {
        ...
        let ecl = self.data_frame["EquivalentChainLength"].list()?;
        ...
        plot = plot
            .x_axis_label(ui.localize("retention-time"))
            .y_axis_label(ui.localize("equivalent-chain-length"))
            .label_formatter(|name, PlotPoint { x, y }| {
        ...
        plot.show(ui, |ui| -> PolarsResult<()> {
            for index in 0..self.data_frame.height() {
                ...
                let Some(ecl) = ecl.get_as_series(index) else {
                    polars_bail!(NoData: "EquivalentChainLength");
                };
